def call() {
    ...
    owaspImage.withRun("-u root --network=mariored -v owasp_data:/zap/wrk/:rw owasp/zap2docker-stable /usr/bin/tail -f /dev/null") { c ->
        sh "docker exec ${c.id} zap-baseline.py -t http://marioapp:3001 -r report_baseInline.html || true"
        sh "docker exec ${c.id} zap-full-scan.py -t http://marioapp:3001 -r report_fullScan.html || true"
