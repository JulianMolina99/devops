def call() {
    ...
    owaspImage.withRun("-u root --network=${env.JOB_NAME}_network_nodejs_app -v owasp_data:/zap/wrk/:rw owasp/zap2docker-stable /usr/bin/tail -f /dev/null") { c ->
        sh "docker exec ${c.id} zap-baseline.py -t http://test_react:3000 -r report_baseInline.html || true"
        sh "docker exec ${c.id} zap-full-scan.py -t http://test_react:3000 -r report_fullScan.html || true"
